...
const processQueue = async () => {
  ...
  while (eventQueue.length > 0) {
    ...
    try {
      ...
      if (isBuy && amountInUSD >= 500) { // Ensure amountInUSD meets minimum threshold
...
const checkForEvents = async (routerContract) => {
  try {
    ...
    if (latestBlock > lastCheckedBlock) {
      ...
      console.log(`Camelot Monitor -> Checking blocks from ${lastCheckedBlock + 1n} to ${endBlock} : ${getTime()}`);
